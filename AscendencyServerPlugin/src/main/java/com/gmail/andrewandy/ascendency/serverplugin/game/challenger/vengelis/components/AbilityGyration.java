package com.gmail.andrewandy.ascendency.serverplugin.game.challenger.vengelis.components;
...
public class AbilityGyration extends AbstractCooldownAbility {
    ...
    @AssistedInject AbilityGyration(@Assisted final Challenger toBind,
                                    final PlayerMatchManager matchManager) {
        super("Gyration", true, 10, TimeUnit.SECONDS, toBind);
        this.matchManager = matchManager;
    }

    @Listener(order = Order.LAST) public void onActivePressed(final ActiveKeyPressedEvent event) {
    ...
    private void executeAsPlayer(final Player player) {
        ...
        final PotionEffect effect = (PotionEffect) new BuffEffectEntangled(1, 0); //Entanglement 1 | Raw cast is fine because of sponge mixins
