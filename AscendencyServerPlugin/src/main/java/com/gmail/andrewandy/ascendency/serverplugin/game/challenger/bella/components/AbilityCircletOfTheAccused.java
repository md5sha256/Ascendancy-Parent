...
public class AbilityCircletOfTheAccused extends AbstractCooldownAbility {
    ...
    private final CircletDataFactory factory;
    ...
    @AssistedInject AbilityCircletOfTheAccused(@Assisted final Challenger bound,
                                               final CircletDataFactory factory,
    ...
    public void activateAs(final UUID caster, final UUID targetUID, final int radius,
        ...
        registeredMap.compute(target.getUniqueId(), (playerUID, circletData) -> {
            if (circletData == null) {
                circletData =
                    factory.create(caster, radius); //Caster is the playerUID, default radius = 3
            }
            circletData.reset();
    ...
    @Listener(order = Order.DEFAULT) public void onJump(final CustomEvents.PlayerJumpEvent event) {
     ...
     */
    public static class ProcEvent extends AscendencyServerEvent implements Cancellable {
